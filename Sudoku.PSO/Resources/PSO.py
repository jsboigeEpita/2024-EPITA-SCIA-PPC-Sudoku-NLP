...
class Solver:
    def __init__(
        ...
        """
        rhop (cognitive coefficient): cognitive influence of an individual particle's best-known position on its own movement
        rhog (social coefficient): represents the social influence of the global best-known position on each particle's movement
        K (inertia weight): determines the particle's tendency to follow its current velocity
        """
    ...
    def init_particle(self):
        ...
        return Particle(
            grid=grid,
            velocity=np.random.uniform(-2, 2, size=81),
            best_score=compute_error(grid),
        )
    ...
    def init_swarm(self):
        ...
        for i in range(self.nb_particles):
            ...
            scores = [
                swarm_ps[0].best_score,
                swarm_ps[1].best_score,
                swarm_ps[2].best_score,
            ]

            swarm = Swarm(
                particles=swarm_ps,
                best=swarm_ps[scores.index(min(scores))].best,
    ...
    def solve(self) -> np.ndarray:
        """
        Solve the sudoku with a main loop which stops when a solution is found
        ...
        while True:
            for swarm in self.swarms:
                for particle in swarm.particles:
                    ...
                    particle.velocity = self.K * (
                        particle.velocity
                        + (
                            self.rhoP
                            * np.random.uniform(size=81)
                    ...
                    for i in range(81):
                        if i not in self.pre_filled_indexes:
                            if (
                                particle.zeros_velocity_count[i] > 500
                            ...
                            if (
                                np.random.uniform()
                                < 2 / (1 + np.exp(-np.abs(particle.velocity[i]))) - 1
                            ):  # scaling function
                                # Excluding the initial value
                                particle.grid[i] = np.random.choice(
                                    [j for j in range(1, 10) if j != particle.grid[i]]
                                )
...
np.random.seed()
solver = Solver(
    np.array(instance).flatten(), K=0.7298, rhoP=0.25, rhoG=0.25, nb_particles=100
)
solver.init_swarm()
result = solver.solve()
